...
def dict_cmp(dict1, dict2, d1name, d2name, required_fields=None):
    ...
    while stack1 and stack2:
        elem1, path1 = stack1.pop(0)
        elem2, path2 = stack2.pop(0)
        e_ret = ret_stack.pop(0)

        if elem1 != elem2:
            if elem1.__class__ != elem2.__class__ \
               and not (isstring(elem1) and
                        isstring(elem2)):
                e_ret['__TYPE_%s__' % d1name] = str(type(elem1))
                e_ret['__TYPE_%s__' % d2name] = str(type(elem2))
            elif isinstance(elem1, dict):
                cmp_dict_object(diff_setup(elem1, elem2, e_ret, stack1,
                                           stack2, ret_stack, path1, path2,
                                           required_fields, d1name, d2name))
            elif isinstance(elem1, list):
                cmp_list_object(diff_setup(elem1, elem2, e_ret, stack1,
                                           stack2, ret_stack, path1, path2,
                                           required_fields, d1name, d2name))
            else:
                e_ret['MISSING_IN_%s' % d1name] = elem2
                e_ret['MISSING_IN_%s' % d2name] = elem1
    if not ret:
        return ret
